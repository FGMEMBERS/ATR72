...
var gpsSearch = func(query) {
	...
	# id, name, lat, lon, type, brg, dist
	
	
	
	# OLD CODE COMMENTED BECAUSE OTHERS MESSED WITH FG GPS CODE >.<
	
	var results = [];
	
	foreach(var type; query_types) {
	
		var gpsResults = atr.gps.searchIdent(query, type);
		
		if (size(gpsResults) > 0) {
			var result = gpsResults[0]; # Just get first result for now
			
			var position = geo.Coord.new();
			position.set_latlon(result.lat, result.lon);
			...
			retResult.brg = int(ac.course_to(position));
			retResult.dist = int(ac.distance_to(position)/1852);
			retResult.type = type;

			append(results, retResult);
		}
